...
export default function HeroSection({ video, showHeader = true, onMenuToggle, menuOpen = false, onVideoLoaded }: HeroSectionProps) {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // If mouse crosses past center-left area, reset immediately
    const resetBoundary = window.innerWidth * 0.45 // 45% from left
    if (e.clientX > resetBoundary) {
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden">
      ...
      {showHeader && videoLoaded && (
        <header className={`fixed top-0 right-0 left-0 px-8 lg:px-20 pt-8 pb-16 transition-all duration-300 ${showHeader ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'} z-40`} dir="rtl" style={{ textAlign: 'right' }}>
          <Link href="/" className="max-w-7xl block hover:opacity-80 transition" style={{ textAlign: 'right', display: 'block' }}>
            <h1 className="text-2xl font-light tracking-widest text-white" style={{ fontFamily: '"Bebas Neue", sans-serif', fontWeight: 400, textAlign: 'right' }}>SHARON MOSHE ATTIAS</h1>
          </Link>
        </header>
      )}
