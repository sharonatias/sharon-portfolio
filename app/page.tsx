        ...
        <div className="max-w-full mx-auto px-12 lg:px-24 pt-4 pb-24">
          ...
          <StaggerContainer>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
              {(() => {
                const exploringIds = pageContent?.exploring_ids?.split(',').filter(Boolean) || []

                if (exploringIds.length > 0) {
                  return brandCaseStudies.filter(cs => exploringIds.includes(cs.id))
                }
