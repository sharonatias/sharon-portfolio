  ...
  return (
    <div className="bg-white text-black overflow-x-hidden">
        ...
        <section className="min-h-screen flex flex-col items-center px-4 sm:px-8 lg:px-20 bg-gradient-to-b from-white to-gray-50 -mt-12 sm:-mt-96 lg:-mt-[28rem] pt-12 sm:pt-80 lg:pt-96 -mb-12 sm:-mb-96">
          <div className="max-w-7xl mx-auto w-full flex flex-col">
            ...
            <div className="w-full flex items-center justify-center text-center">
              <p className="text-sm sm:text-base lg:text-xl font-light text-gray-800 leading-relaxed max-w-md">
                A spectrum-driven palette expressing energy and optimism.
              </p>
            </div>
          </div>
        </section>
      ...
      {(() => {
        ...
        return (
          <section key={sectionKey} className={`${isFullWidthImage ? 'h-auto' : isShapeSection ? 'h-96 sm:h-[650px]' : 'min-h-auto sm:min-h-screen'} flex flex-col sm:flex-row ${isFullWidthImage ? 'py-0 -my-0 sm:-my-12' : isShapeSection ? 'py-0 -mt-12 sm:-mt-24' : 'py-8 sm:py-12'} ${isSystemSection ? 'pb-0' : ''} ${sectionIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
            {/* Images Column */}
            {hasImages && !isShapeSection && (
              <div className={`${isFullWidthImage ? 'w-full' : isShapeSection ? 'w-full sm:w-3/5 h-full bg-black' : 'w-full sm:w-1/2'} flex items-center justify-center overflow-hidden order-2 ${isFullWidthImage && isImageLeft ? 'sm:order-1' : ''}`}>
                <div className={`w-full ${isShapeSection ? 'h-full' : ''} flex items-center justify-center ${isFullWidthImage ? 'p-0' : isShapeSection ? 'p-0' : 'p-8'}`}>
                  <div className={`${isApplicationsSection ? `grid ${applicationsGridClass} w-full gap-2 sm:gap-4` : isFullWidthImage ? (hasTwoImages ? 'grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 w-full' : 'grid grid-cols-1 gap-0 w-full') : isShapeSection ? 'grid grid-cols-1 sm:grid-cols-2 gap-2 w-full' : 'grid grid-cols-2 sm:grid-cols-4 gap-2 w-full max-w-4xl'}`}>
                    {section.images.map((img: any, idx: number) => {
                      const imageId = `${sectionKey}-${idx}`
                      const imgUrl = typeof img === 'string' ? img : img.url
                      return (
                        <div key={idx} className={isApplicationsSection ? "w-full h-auto overflow-hidden rounded-lg" : isFullWidthImage ? "w-full h-auto" : isShapeSection ? "w-full h-full" : "aspect-square overflow-hidden rounded-lg"}>
                          <img
                            src={imgUrl}
                            alt={`${sectionLabel} ${idx + 1}`}
                            className={`${isApplicationsSection ? 'w-full h-auto object-contain' : isFullWidthImage ? 'w-full h-auto' : isShapeSection ? 'w-full h-full object-contain' : 'w-full h-full'} object-contain`}
                            loading="lazy"
                          />
                        </div>
                      )
            ...
            <div className={`${hasImages ? 'w-full sm:w-1/2' : 'w-full'} flex items-center justify-center px-4 sm:px-6 lg:px-16 py-6 sm:py-12 lg:py-24 order-1 ${isImageLeft ? 'sm:order-2' : 'sm:order-1'}`}>
