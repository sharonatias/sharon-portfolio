      ...
      {(() => {
        ...
        return (
          <section key={sectionKey} className={`${isFullWidthImage ? 'h-auto' : isShapeSection ? 'h-96 sm:h-[650px]' : 'min-h-auto sm:min-h-screen'} flex flex-col sm:flex-row ${isFullWidthImage ? 'py-0 -my-0 sm:-my-12' : isShapeSection ? 'py-0 -my-2' : 'py-8 sm:py-12'} ${isSystemSection ? 'pb-0' : ''} ${sectionIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
            {/* Images Column */}
            {hasImages && (
              <div className={`${isFullWidthImage ? 'w-full' : isShapeSection ? 'w-full sm:w-3/5 h-full' : 'w-full sm:w-1/2'} flex items-center justify-center overflow-hidden order-2 ${isFullWidthImage && isImageLeft ? 'sm:order-1' : ''}`}>
                      ...
                      return (
                        <div key={idx} className={isApplicationsSection ? "w-full h-auto overflow-hidden rounded-lg" : isFullWidthImage ? "w-full h-auto" : isShapeSection ? "w-full h-full" : "aspect-square overflow-hidden rounded-lg"}>
                          <img
                            ...
                            className={`${isApplicationsSection ? 'w-full h-auto object-contain' : isFullWidthImage ? 'w-full h-auto' : isShapeSection ? 'w-full h-full object-cover' : 'w-full h-full'} object-contain`}
                            loading="lazy"
                          />
                        </div>
                      )
